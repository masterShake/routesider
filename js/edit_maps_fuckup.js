...
(function(){
	...
	MA = function( map ){
		...
		// this.pinDropper = new PinDropper( this );
	...
	var PinDropper = function( ma ){

		/* properties */

		// pointer to the parent MA object
		this.mapApp = ma;
		// pointer directly to the googleMap obj.
		this.map = ma.map;
	    ...
	    document.getElementById("components-toolbar").children[3]
	    	.addEventListener("click", mapApp.pinDropper.toggleNewPinMode, false);

	    // add event listener to the close new pin toolbar x
	    document.getElementById("drop-new-pin-toolbar").children[0]
	    	.addEventListener("click", mapApp.pinDropper.terminateNewPinMode, false);
	}
	...
	PinDropper.prototype.initNewPinMode = function(){
		...
		this.ma.termFormattingModes("new pin");

		// set drop-new-pin mode property to true
		this.ma.newPinMode = true;
	...
	PinDropper.prototype.initGooglePinDrop = function(){
		...
		google.maps.event.addListener( this.map, 'click', mapApp.pinDropper.dropPinClick );
	...
	PinDropper.prototype.dropPinClick = function(e){
		...
		mapApp.pinDroppertempLatLng = e.latLng;
	...
	PinDropper.prototype.pinDropAutocomp = function(){
		...
		mapApp.map.panTo( mapApp.tempLatLng );

		// zoom to the correct resolution
		mapApp.map.setZoom( 15 );
	...
	PinDropper.prototype.dropPin = function(){
		...
		this.ma.pins.push( new google.maps.Marker({
	      position: this.tempLatLng,
	      map: this.map,
	      title: 'Hello World!',
	...
	init =  function(){ console.log("we have called the initialization");
		...
		window.mapApp = new MA( rsApp.map );

		// mapApp.pinDropper = new PinDropper( mapApp );
	}
